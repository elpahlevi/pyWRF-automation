...
# Download GFS file of Indonesia on 0.25 degree grid resolution
def downloadGFS(issued_time, forecast_time):
    base_URL = "https://nomads.ncep.noaa.gov/cgi-bin/"
    url_query = {
        "elev" : "&all_lev=on", 
        "var" : "&all_var=on", 
        "region" : "&subregion=&leftlon=95&rightlon=141&toplat=6&bottomlat=-11"
    }
    
    #Generate the urls and file name based on hours (000 to 384 hours)
    hours = ["%03d" % hrs for hrs in range(0, forecast_time, 3)]
    get_URL = [base_URL + "filter_gfs_0p25.pl?file=gfs.t"+ issued_time +"z.pgrb2.0p25.f"+ "%03d" % hour + url_query.get("elev") + url_query.get("var") + url_query.get("region") + "&dir=%2Fgfs." + year + month + day + "%2F" + issued_time for hour in range(0, forecast_time, 3)]
    files_name = ["gfs_4_"+ year + month + day + "_" + issued_time + "00" +"_"+ "%03d" % hour +".grb2" for hour in range(0, forecast_time, 3)]
    ...
    print("GFS files will be saved in {}".format(folder_location))
    ...
    for url, filename, hours in zip(get_URL, files_name, hours):
        ...
        with open(folder_location + filename, 'wb') as f:
            ...
            curl_download.close()
        print("GFS file on " + day + "-" + month + "-" + year + " issued time " + issued_time + " and forecast time +" + str(hours) +  " has been downloaded.")
    print("All GFS files with issued time {} downloaded".format(issued_time))
...
def runWPS():
    ...
    gfs_date_folder = start_date + relativedelta(days=0)
    gfs_date_folder = gfs_date_folder.strftime("%Y-%m-%d")
...
def runWRF():
    ...
    #execute real.exe
    subprocess.run(["mpirun -np 4 ./real.exe"], shell=True, cwd=dir + "/WRF/test/em_real/")
    ...
    if re.search("SUCCESS COMPLETE REAL_EM INIT", str(rsl_error)):
        #execute wrf.exe
        subprocess.run(["mpirun -np 4 ./wrf.exe"], shell=True, cwd=dir + "/WRF/test/em_real/")
        print("SUCCESS COMPLETE REAL_EM INIT")
    else:
        print("Check namelist.input")
#Run the Code
downloadGFS(issued_time = "00", forecast_time = 193)
